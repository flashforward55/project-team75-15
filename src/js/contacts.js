...
let eyeGLTF =
	"https://devloop01.github.io/so-many-eyes/public/models/eye_model.glb";
...
class Gl {
	...
	addEyes() {
		...
		const refGeo = new PlaneBufferGeometry(
			EYE_GAP,
			EYE_GAP,
			EYE_AMOUNT - 1,
			EYE_AMOUNT - 1
	...
	initAnimation() {
		const getDir = (v) =>
			v.clone().sub(new Vector3(0, 0, INTERSECTION_PLANE_Z)).normalize();
		...
		if (this.eyes && this.animationComplete)
			this.eyes.forEach((eye, index) => {
				...
				let dir = pos
					...
					.sub(this.mouse3d)
					.normalize()
					.multiplyScalar(POSITION_OFFSET_FACTOR);
